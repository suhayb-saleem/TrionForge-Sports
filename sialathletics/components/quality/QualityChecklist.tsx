    ...
    <section style={{ background: 'var(--bg-base)', padding: '6rem 1.5rem' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }}>
        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7 }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 4vw, 3rem)', color: 'var(--white)', lineHeight: 0.95, margin: '1rem 0 2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {checks.map((check, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                ...
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.95rem', color: 'var(--white-90)', lineHeight: 1.5 }}>{check}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7, delay: 0.1 }} style={{ aspectRatio: '4/3', background: 'var(--bg-card)', border: '1px solid var(--white-08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: 'var(--white-60)', fontSize: '0.75rem', fontFamily: 'var(--font-body)', letterSpacing: '0.1em' }}>QC PHOTO</span>
        </motion.div>
      </div>
    </section>
  );
