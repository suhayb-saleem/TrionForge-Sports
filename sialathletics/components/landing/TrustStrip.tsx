...
export default function TrustStrip() {
  return (
    <div style={{ background: 'var(--bg-raised)', borderTop: '2px solid var(--red)', borderBottom: '1px solid var(--white-08)' }}>
      <div className="trust-strip-grid" style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid' }}>
        {items.map(({ Icon, label }, i) => (
          <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 + i * 0.08 }}
            ...
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--white-60)' }}>{label}</span>
          </motion.div>
        ))}
      </div>
      <style>{`
        .trust-strip-grid { grid-template-columns: repeat(4, 1fr); }
        .trust-strip-item { border-right: 1px solid var(--white-08); }
        .trust-strip-item:last-child { border-right: none; }
        @media (max-width: 768px) {
          .trust-strip-grid { grid-template-columns: repeat(2, 1fr); }
          .trust-strip-item:nth-child(2) { border-right: none; }
          .trust-strip-item:nth-child(1), .trust-strip-item:nth-child(2) { border-bottom: 1px solid var(--white-08); }
        }
        @media (max-width: 480px) {
          .trust-strip-grid { grid-template-columns: 1fr; }
          .trust-strip-item { border-right: none; border-bottom: 1px solid var(--white-08); }
