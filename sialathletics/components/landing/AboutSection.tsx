    ...
    <section style={{ background: 'var(--bg-light)', padding: '8rem 1.5rem', borderTop: '1px solid var(--border-light)' }}>
      <div className="about-section-grid" style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gap: '4rem', alignItems: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem, 4.5vw, 4rem)', color: 'var(--text-dark)', lineHeight: 0.9, margin: '1.25rem 0 1.5rem' }}>
            YOUR EDGE STARTS<br />AT THE SOURCE.
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', lineHeight: 1.8, color: 'var(--text-muted)', marginBottom: '2.5rem', maxWidth: '480px' }}>
          ...
          <Link href="/about" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-dark)', fontFamily: 'var(--font-body)', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', borderBottom: '2px solid var(--red)', paddingBottom: '3px', transition: 'border-color 0.2s ease' }}>
            OUR STORY →
          </Link>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
          style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: 'var(--border-light-strong)' }}>
          {stats.map(({ to, label, duration, isM }) => (
            <div key={label} style={{ background: 'var(--bg-light-alt)', padding: '2.5rem 2rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem, 4vw, 3.5rem)', color: 'var(--text-dark)', lineHeight: 1, letterSpacing: '0.02em' }}>
