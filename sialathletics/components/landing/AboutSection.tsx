    ...
    <section style={{ background: 'var(--bg-raised)', padding: '8rem 1.5rem', borderTop: '1px solid var(--white-08)' }}>
      <div className="about-section-grid" style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gap: '4rem', alignItems: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem, 4.5vw, 4rem)', color: 'var(--white)', lineHeight: 0.9, margin: '1rem 0 1.5rem' }}>
            YOUR EDGE STARTS<br />AT THE SOURCE.
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', lineHeight: 1.8, color: 'var(--white-60)', marginBottom: '2rem', maxWidth: '480px' }}>
          ...
          <Link href="/about" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: 'var(--white)', fontFamily: 'var(--font-body)', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', borderBottom: '1px solid var(--white-30)', paddingBottom: '3px', transition: 'border-color 0.2s ease' }}>
            OUR STORY →
          </Link>
        </motion.div>
