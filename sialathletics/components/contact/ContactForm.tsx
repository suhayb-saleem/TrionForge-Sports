      ...
      <div className="contact-form-grid" style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gap: '4rem', alignItems: 'start' }}>
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} style={{ background: 'var(--bg-card)', border: '1px solid var(--white-08)', padding: '2.5rem' }}>
          <div className="contact-fields-grid" style={{ display: 'grid', gap: '1.25rem' }}>
            {['Your Name *', 'Work Email *', 'Company Name', 'Country / Region'].map((field) => (
              <div key={field} style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                <input type="text" style={{ background: 'var(--bg-raised)', border: '1px solid var(--white-08)', padding: '0.75rem 1rem', color: 'var(--white)', fontFamily: 'var(--font-body)', fontSize: '0.875rem', outline: 'none', width: '100%' }} />
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginTop: '1.25rem' }}>
            ...
            <textarea rows={4} style={{ background: 'var(--bg-raised)', border: '1px solid var(--white-08)', padding: '0.75rem 1rem', color: 'var(--white)', fontFamily: 'var(--font-body)', fontSize: '0.875rem', outline: 'none', resize: 'vertical', width: '100%' }} />
          </div>
          <button style={{ marginTop: '1.5rem', width: '100%', background: 'var(--red)', color: 'var(--white)', border: 'none', padding: '1rem', fontFamily: 'var(--font-body)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 600, cursor: 'pointer' }}>
            SUBMIT INQUIRY
          </button>
